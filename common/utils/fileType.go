...
func init() {
	...
	fileTypeMap.Store("424d228c010000000000", "bmp")  //16 bit(bmp)
	fileTypeMap.Store("424d8240090000000000", "bmp")  //24 bit(bmp)
	fileTypeMap.Store("424d8e1b030000000000", "bmp")  //256 bit(bmp)
	...
	fileTypeMap.Store("d0cf11e0a1b11ae10000", "doc")  //MS Excel (word、msi)
	fileTypeMap.Store("d0cf11e0a1b11ae10000", "vsd")  //Visio
	...
	fileTypeMap.Store("2e524d46000000120001", "rmvb") //rmvb (rm)
	fileTypeMap.Store("464c5601050000000900", "flv")  //flv (f4v)
	...
	fileTypeMap.Store("3026b2758e66cf11a6d9", "wmv") //wmv (asf)
	...
	fileTypeMap.Store("4d616e69666573742d56", "mf")         //MF
	fileTypeMap.Store("3c3f786d6c2076657273", "xml")        //xml
	fileTypeMap.Store("494e5345525420494e54", "sql")        //xml
	fileTypeMap.Store("7061636b616765207765", "java")       //java
	fileTypeMap.Store("406563686f206f66660d", "bat")        //bat
	fileTypeMap.Store("1f8b0800000000000000", "gz")         //gz
	fileTypeMap.Store("6c6f67346a2e726f6f74", "properties") //bat
	fileTypeMap.Store("cafebabe0000002e0041", "class")      //bat
	fileTypeMap.Store("49545346030000006000", "chm")        //bat
	fileTypeMap.Store("04000000010000001300", "mxp")        //bat
	fileTypeMap.Store("504b0304140006000800", "docx")       //docx
	fileTypeMap.Store("d0cf11e0a1b11ae10000", "wps")        //WPS
...
func GetFileType(fSrc string) (string, error) {
	...
	f, err := os.ReadFile(fSrc)
	if err != nil {
		return "", err
	}

	fileCode := bytesToHexString(f)
	...
	if fileType == "" {
		err = errors.New("Unknoen file type")
	}
	return fileType, err
}
