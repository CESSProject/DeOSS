...
func (c *chainClient) FileSecreach(filehash string, user UserBrief) (string, error) {
	defer func() { recover() }()
	var (
		txhash      string
		accountInfo types.AccountInfo
	...
	for {
		...
		case status := <-sub.Chan():
			if status.IsInBlock {
				...
				if len(events.FileBank_FlyUpload) > 0 {
					return txhash, nil
				}
				return txhash, errors.New(ERR_Failed)
			}
...
func (c *chainClient) ExitOss() (string, error) {
	defer func() { recover() }()
	var (
		txhash      string
		accountInfo types.AccountInfo
